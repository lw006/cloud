...
class Role
{
    ...
    {
        //分组信息
        $list_group = Loader::model('Group')->getList();
        ...
        if (!ADMIN) {
            $access_node = Db::name("Access")->alias("access")->join("__ROLE_USER__ role_user", "role_user.role_id = access.role_id")->field("access.node_id")->where("role_user.user_id", UID)->select();
            $where_node['id'] = ["in", filter_value($access_node, "node_id")];
        }
        $node = Db::name("Node")->where("status=1 AND isdelete=0")->field("id,pid,group_id,name,title,level,type")->select();
        $accesses = Db::name("Access")->where("role_id", $role_id)->select();
